...
class CoupleRecipeApp {
  ...
  async loadAppData() {
    try {
      // 並列でデータを取得
      const [recipes, folders, mealPlans] = await Promise.all([
        DatabaseHelper.getRecipes(),
        DatabaseHelper.getFolders(),
  ...
  showRecipeModal(recipe = null) {
    ...
    this.renderFolderCheckboxes(recipe);
  ...
  async saveRecipe() {
    ...
    try {
      ...
      // フォルダ関連付け
      const selectedFolders = Array.from(document.querySelectorAll('#folder-checkboxes input:checked'))
        .map(checkbox => checkbox.value);

      if (selectedFolders.length > 0) {
        const folderRelations = selectedFolders.map(folderId => ({
          recipe_id: savedRecipe.id,
          folder_id: folderId
        }));

        const { error: relationError } = await supabaseClient
          .from('recipe_folder_relations')
          .insert(folderRelations);

        if (relationError) {
          console.warn('フォルダ関連付けエラー:', relationError);
        }
      }
  ...
  renderRecipes() {
    ...
    const recipesHtml = this.recipes.map(recipe => `
      <div class="recipe-card" data-recipe-id="${recipe.id}">
        ...
        <div class="recipe-folders">
          ${(recipe.recipe_folder_relations || []).map(rel => 
            `<span class="folder-tag" style="background-color: ${rel.recipe_folders.color}">
              ${this.escapeHtml(rel.recipe_folders.name)}
            </span>`
          ).join('')}
        </div>
      </div>
  ...
  renderFolderFilters() {
    ...
    let filtersHtml = '<button class="filter-btn active" data-folder="all">すべて</button>';
    
    this.folders.forEach(folder => {
      filtersHtml += `
        <button class="filter-btn" data-folder="${folder.id}" style="border-color: ${folder.color}">
    ...
    container.querySelectorAll('.filter-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        this.filterRecipesByFolder(btn.dataset.folder);
      });
    });
  }

  renderFolderCheckboxes(recipe = null) {
    const container = document.getElementById('folder-checkboxes');
    
    let checkboxesHtml = '';
    
    this.folders.forEach(folder => {
      const isChecked = recipe && recipe.recipe_folder_relations?.some(rel => 
        rel.recipe_folders.id === folder.id
      );
      
      checkboxesHtml += `
        <label class="checkbox-label">
          <input type="checkbox" value="${folder.id}" ${isChecked ? 'checked' : ''}>
          <span class="checkbox-text" style="color: ${folder.color}">
            ${this.escapeHtml(folder.name)}
          </span>
        </label>
      `;
  ...
  filterRecipesByFolder(folderId) {
    const cards = document.querySelectorAll('.recipe-card');
    ...
    cards.forEach(card => {
      if (folderId === 'all') {
        ...
        const hasFolder = recipe?.recipe_folder_relations?.some(rel => 
          rel.recipe_folders.id === folderId
        );
